...
uint16_t count = 0;
byte last_ch[7];
bool flag_arm = 0, failsafe = 0, flag_accelval = 0, flag_calib = 0;
int16_t x = 1000;
...
void setup()
{
  ...
  for (i = 1; i < 7; i++)
  {
    rfmin[i] = EEPROM.read(2 * i - 1) << 8 | EEPROM.read(2 * i);

    rfmax[i] = EEPROM.read(2 * i + 7) << 8 | EEPROM.read(2 * i + 8);
  ...
  //pid_y.kp=0; pid_y.kd=0; pid_y.ki=0;
  k = 0;                                                                                               // throttle control coefficient
...
delay(1);
   ...
   roll_offset /= i;
 pitch_offset  /=  i;
 //Serial.println((String)roll_offset+"\t"+(String)pitch_offset);
}
...
ISR(PCINT0_vect)    {
  ...
  else if (last_ch[1] == 1 && !(PINB & 0b00000001))
  {
    ...
    // Serial.print(rx_ch[1]);
    if ((rx_ch[1] <= chmin[1]) && (rx_ch[1] > 950))
      chmin[1] = rx_ch[1];
    if ((rx_ch[1] >= chmax[1]) && (rx_ch[1] < 2050))
      chmax[1] = rx_ch[1];


    chm[1] = (((float)(rx_ch[1] - rfmin[1]) / (float)(rfmax[1] - rfmin[1])) * 160) - 80 ; // chm[]= mapped values of channels
    float test = (rx_ch[1] - rfmin[1]), test1 = (rfmax[1] - rfmin[1]);
  ...
  else if (last_ch[2] == 1 && !(PINB & 0b00000010))
  {
    ...
    if ((rx_ch[2] <= chmin[2]) && (rx_ch[2] > 950))
      chmin[2] = rx_ch[2];
    if ((rx_ch[2] >= chmax[2]) && (rx_ch[2] < 2050))
      chmax[2] = rx_ch[2];
    chm[2] = (((float)(rx_ch[2] - rfmin[2]) / (float)(rfmax[2] - rfmin[2])) * 2 *pitch_lim ) - pitch_lim ; // chm[]= mapped values of channels
  }
  ///////////////////             channel 3                     /////////////////////
  ...
  else if (last_ch[3] == 1 && !(PINB & 0b00000100))
  {
    ...
    if ((rx_ch[3] <= chmin[3]) && (rx_ch[3] > 950))
      chmin[3] = rx_ch[3];
    if ((rx_ch[3] >= chmax[3]) && (rx_ch[3] < 2050))
      chmax[3] = rx_ch[3];
    if (rx_ch[3] > rfmin[3])
      chm[3] = (((float)(rx_ch[3] - rfmin[3]) / (float)(rfmax[3] - rfmin[3])) * 1000) + 1000 ; // chm[]= mapped values of channels
    float test = (rx_ch[3] - rfmin[3]), test1 = (rfmax[3] - rfmin[3]) ;
  ...
  else if (last_ch[4] == 1 && !(PINB & 0b00100000))
  {
    ...
    if ((rx_ch[4] <= chmin[4]) && (rx_ch[4] > 950))
      chmin[4] = rx_ch[4];
    if ((rx_ch[4] >= chmax[4]) && (rx_ch[4] < 2050))
      chmax[4] = rx_ch[4];
    chm[4] = (((float)(rx_ch[4] - rfmin[4]) / (float)(rfmax[4] - rfmin[4])) * 160) - 80 ; // chm[]= mapped values of channels
  }
  ///////////////////                 channel 5                 /////////////////////
  /* if(last_ch[5]==0 && (PINB & 0b00000010))           //checking state of digital pin 52
    ...
    else if(last_ch[5]==1 && !(PINB & 0b00000010))
    {
     ...
     if (rx_ch[5]<=chmin[5])
       ...
       if(rx_ch[5]>=chmax[5])
       chmax[5]=rx_ch[5];
        chm[5] = (((rx_ch[5]-chmin[5])/(chmax[5]-chmin[5]))*1000)+1000 ;// chm[]= mapped values of channels
    }
     ///////////////////channel 6 /////////////////////
...
{
  int i;
 // Serial.println("storing rf min and max");
  for (i = 1; i < 5; i++)
  {
   // Serial.println(i * 25);

    EEPROM.update(2 * i, chmin[i]); //Accessing one byte data and then left shifting that data and accessing one more byte,then oring them.This become 16-bit data that we want to store in EEPROM
    EEPROM.update(2 * i - 1, chmin[i] >> 8);
    EEPROM.update(2 * i + 8, chmax[i]);
    EEPROM.update(2 * i + 7, chmax[i] >> 8);
  }
